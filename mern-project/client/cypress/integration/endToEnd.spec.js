describe("Web site availability", () => {
  after(() => {
    // cy.contains("Delete").click({ force: true });

    cy.visit("http://localhost:3000/records");
    cy.contains("Employee2").parent("tr").contains("Delete").click({ force: true });
  });
  it("Sanity listings web site", () => {
    cy.visit("http://localhost:3000");
    cy.contains("Create Record").should("exist");
  });
  it("Test Adding Employee listings", () => {
    cy.visit("http://localhost:3000/create");
    cy.get("#name").type("Employee1");
    cy.get("#position").type("Position1");
    cy.get("#positionIntern").click({ force: true });
    cy.contains("Create person").click({ force: true });
    cy.visit("http://localhost:3000/records");
    cy.contains("Employee1").should("exist");
  });
  it("Test Editing Employee", () => {
    cy.visit("http://localhost:3000/records");
    cy.contains("Employee1").parent("tr").contains("Edit").click();
    cy.get("#name").clear().type("Employee2");
    cy.get("#position").clear().type("Position2");
    cy.get("#positionJunior").click({ force: true });
    cy.get('input[type="submit"][value="Update Record"]').click();
    cy.visit("http://localhost:3000/records");
    cy.contains("Employee2").should("exist");
    cy.contains("Position2").should("exist");
  });
});
