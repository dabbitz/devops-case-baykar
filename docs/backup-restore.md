...
Bu runbook, MongoDB Atlas üzerinde kullanılan `sample_training` database'inin yedeklenmesi ve geri yüklenmesi için kullanılan yöntemi, otomatik Kubernetes backup akışını ve gerçekleştirilen doğrulama senaryosunu açıklamaktadır.
...
| Nasıl çalıştırılıyor / Execution method (script, Job, CronJob, manuel) | EKS ortamında `mongodb-backup` adlı Kubernetes `CronJob` otomatik olarak çalışmaktadır. Schedule `0 2 * * *`, timezone `Europe/Istanbul` şeklindedir. Backup ayrıca repository içerisindeki `scripts/backup-restore.ps1` script'i üzerinden manuel olarak da tekrarlanabilir.                                                                                                                                 |
...
Buradaki `Z` UTC zamanını ifade eder. Örneğin `23:00 UTC`, `Europe/Istanbul` timezone'unda `02:00` değerine karşılık gelir.
...
Backup dosyasının oluşturulmasının ardından `s3-upload` container'ı `aws s3 cp` ile object'i S3'e yükler ve `aws s3api head-object` ile yüklenen object'in erişilebilir olduğunu doğrular.
...
Bu script, daha önce gerçekleştirilen local E2E backup/restore senaryosunda kullanılan `mongodump` yaklaşımını tekrarlanabilir hale getirmektedir.
...
Database silindikten sonra kullanılan test backup'ı `mongorestore` ile geri yüklenmiştir. Aşağıdaki manual command chain kullanılan yöntemi açıkça dokümante etmek amacıyla verilmiştir.
...
1. `mongodump` / `mongodb-backup` çıktısında backup işleminin başarıyla tamamlandığı kontrol edilmiştir.
2. Otomatik backup workload'unda oluşturulan archive dosyasının boş olmadığı `test -s` ile doğrulanmıştır.
3. S3 upload işlemi başarıyla tamamlandıktan sonra `aws s3api head-object` ile ilgili object'in erişilebilir olduğu doğrulanmıştır.
...
6. `sample_training` database'i tamamen silindikten sonra uygulama ve MongoDB Atlas üzerinden verilerin kaybolduğu doğrulanmıştır.
...
Temel backup/restore senaryosu 9 Eylül 2026 tarihinde gerçekleştirilmiştir. Otomatik EKS backup senaryosu ise 13 Eylül 2026 tarihinde scheduler tarafından gerçek ortamda doğrulanmıştır.
...
| 7 | Otomatik scheduled backup / Scheduled backup     | Başarılı. `0 2 * * *` / `Europe/Istanbul` schedule ile `mongodb-backup-29820900` Job'ı `Complete 1/1` olarak tamamlandı ve timestamp'li S3 object'i oluşturuldu. | `docs/screenshots/53-backup-cronjob-scheduled-success.png`                                                                                                                        |

## 7. Bilinen sınırlamalar / Known limitations

Bu case kapsamında temel backup/restore E2E testi manuel olarak gerçekleştirilmiş, ayrıca otomatik EKS backup mekanizması gerçek scheduler çalışması ile doğrulanmıştır.

Mevcut çözümde:

- Backup'lar Amazon S3'te cluster dışı storage olarak tutulmaktadır.
- Backup işlemi her gün 02:00 `Europe/Istanbul` timezone'unda otomatik çalışmaktadır.
...
- Backup object'lerinin encryption, retention ve lifecycle yönetimi gerçek production ortamında iş gereksinimi ve compliance gereksinimlerine göre ayrıca yapılandırılmalıdır.
- Düzenli restore tatbikatları ve daha düşük RPO gereksinimlerinde point-in-time recovery veya daha sık backup yaklaşımı değerlendirilmelidir.
